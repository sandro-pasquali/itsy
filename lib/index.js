...
Publisher.prototype.send = function(topic, obj) {
	return new Promise(function(resolve, reject) {
		...
		if(!sv.available) {
			return sv.sendQueue.push(_send);
		}
	...
	if(typeof fn === "string") {
		...
		if(stats.isDirectory()) {
			...
			exec('(cd ' + mpath + '; npm i; npm test)', function(error, stdout) {
				...
				service.on('message', function(pid) {

					SERVICES[this.topic].available = service;
					SERVICES[this.topic].sendQueue.forEach(function(f) {
						f();
					});
					SERVICES[this.topic].sendQueue = [];
