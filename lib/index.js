...
_completeSub.on('message', function(payload) {
	...
	try {
		...
		//	Log when we receive a non-parseable response
		//
		throw e;
	}
...
function Subscriber(publisher, topic) {
	...
	this.sub.identity = 'subscriber_' + process.pid
...
module.exports = function(port) {
	...
	publisher.identity = 'publisher_' + process.pid;
	var api = {
		...
		send: function(serviceName, data, cb) {
			...
			return publisher.send(serviceName, data, cb);
		}
	};
	return api;
}
