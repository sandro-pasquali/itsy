...
//	The dirname of the file that required this module.
//	@see	Subscriber#use
//
...
var SERVICES = {
	...
	completeLoading : function(topic, key, pid) {
		...
		if(--stt.loading === 0) {
			stt.sendQueue.forEach(function(f) {
				console.log('(fn)runner: ' + f.displayName);
				f();
			});
			stt.sendQueue = [];
		}
	...
	handleServiceSubscribed : function(router, identity, payload) {
		//	id, topic, key, pid
		//
		var sp = payload.split(' ');
		return this.completeLoading(sp[1], sp[2], sp[3]);	
	},
	shouldStart : function(list, procName) {
		var i = 0;
		for(; i < list.length; i++) {
			console.log(list[i].name, ' === ', procName)
...
function SET_COMPLETION_HANDLER(publisher) {
	...
	publisher.router.on('message', function(identity, payload) {
		...
		if(id === "SERVICE_SUBSCRIBED") {
			return SERVICES.handleServiceSubscribed(publisher.router, identity, payload);
		}
...
Subscriber.prototype.use = function(servicePath) {
	...
	//	Paths are relative to directory of file that required this module.
	//
	var mpath = path.resolve(CALLER_DIR, servicePath);

	//	The argument last sent to #fulfill (a targeted null key)
	//
	var lkg = self.lastKeyGroup;

	//	Forward-slash separated identifier:
	//	<topic>/<key>/<last 2 segments of service module path>
	//
	var procName = path.join.apply(_ ,
		[self.topic, lkg].concat(mpath.split('/').splice(-2))
	);
	...
	exec('(cd ' + mpath + '; npm i; npm test)', function(error, stdout) {
		...
		pm2.connect(function() {
			pm2.list(function(err, list) {
				if(SERVICES.shouldStart(list, procName)) {
					pm2.start(mpath, { 
						name: procName,
						scriptArgs: [
...
module.exports = function(opts) {
	...
	//	Test runner is calling.
	//
	if(typeof opts === 'function') {
		return require('./serviceHarness')(opts);
	}
